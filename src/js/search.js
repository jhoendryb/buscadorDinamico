class Search {
    ...
    init() {
        ...
        if (!this.searchInput.parentNode.classList.contains("content-search")) {
            this.searchInput.parentNode.classList.add("content-search");
        }

        // EN CASO DE QUE NO EXISTA EL CONTENEDOR PRINCIPAL LO GUARDAMOS
        if (!this.headerElement) {
            this.headerElement = this.searchInput.parentNode;
        }

        // EN CASO DE QUE NO EXISTA EL CONTENEDOR PRINCIPAL LO GUARDAMOS
        if (!this.parent_container) {
            this.parent_container = this.searchInput.parentNode.parentNode;
        }

        // EN CASO DE QUE NO EXISTA EL CONTENEDOR DE ITEMS LO GUARDAMOS
        if (!this.mainElement) {
            if (!this.parent_container.querySelector("main")) {
                this.mainElement = document.createElement("main");
                this.mainElement.classList.add("content-items");
                this.parent_container.appendChild(this.mainElement);
            }

            this.mainElement = this.parent_container.querySelector(".content-items");
        }

        // OBTENEMOS LA PAGINACION PARA SABER SI DEBE USARSE
        let { pagination } = this.options;

        // SI ES TRUE APLICAMOS LA PAGINACION
        if (pagination) {
            let { parent_container, paginationContent } = this;
