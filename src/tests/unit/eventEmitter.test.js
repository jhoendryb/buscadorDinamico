...
describe('EventEmitter', () => {
    ...
    test('debe emitir evento sin listeners sin error', () => {
        const emitter = new EventEmitter();

        expect(() => emitter.emit('test', { data: 'hola' })).not.toThrow();
    });

    test('debe emitir evento con múltiples argumentos', () => {
        const emitter = new EventEmitter();
        const mockCallback = jest.fn();

        emitter.on('test', mockCallback);
        emitter.emit('test', { data: 'hola', extra: 'info' });
